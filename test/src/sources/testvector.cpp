...
void TestVector::testSize() {
    TS_ASSERT_EQUALS(vINT.size(), 3);
    TS_ASSERT_EQUALS(vDOUBLE.size(), 4);
    TS_ASSERT_EQUALS(vCHAR.size(), 2);
    TS_ASSERT_EQUALS(vBOOL.size(), 5);
...
void TestVector::testAt() {
    ...
    TS_ASSERT_EQUALS(vBOOL.at(1), false);
...
void TestVector::testFront() {
    ...
    TS_ASSERT_EQUALS(vDOUBLE.front(), 1.2);
    TS_ASSERT_EQUALS(vCHAR.front(), 'a');
    TS_ASSERT_EQUALS(vBOOL.front(), true);
    TS_ASSERT_EQUALS(vSTRING.front(), "so");
}

/* method for testing function Vector<T>::back() */
void TestVector::testBack() {
    TS_ASSERT_EQUALS(vINT.back(), 5);
    TS_ASSERT_EQUALS(vDOUBLE.back(), 4.4);
    TS_ASSERT_EQUALS(vCHAR.back(), 'c');
    TS_ASSERT_EQUALS(vBOOL.back(), true);
    TS_ASSERT_EQUALS(vSTRING.back(), "cool");
}
...
void TestVector::testOperator() {
    ...
    TS_ASSERT_EQUALS(vCHAR[2], 'c');
    TS_ASSERT_EQUALS(vBOOL[1], true);
    TS_ASSERT_EQUALS(vSTRING[0], "programming");
}
...
void TestVector::testPushBack() {
    ...
    TS_ASSERT_EQUALS(vDOUBLE[4], 4.4);
    TS_ASSERT_EQUALS(vCHAR[4], 'd');
    TS_ASSERT_EQUALS(vBOOL[4], true);
